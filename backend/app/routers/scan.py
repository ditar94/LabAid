...
def _build_cocktail_scan_result(
    ...
    source_lots = {sl.id: sl for sl in db.query(Lot).filter(Lot.id.in_(source_lot_ids)).all()} if source_lot_ids else {}
    ...
    for s in sources:
        ...
        ab = abs_map.get(comp.antibody_id) if comp and comp.antibody_id else None
...
def scan_lookup(
    ...
    if not lot:
        ...
        if cocktail_lot:
            return _build_cocktail_scan_result(db, cocktail_lot)

        # Fallback: search CocktailRecipe by name (case-insensitive)
        recipe_match = db.query(CocktailRecipe).filter(
            CocktailRecipe.name.ilike(body.barcode.strip()),
        )
        if target_lab_id:
            recipe_match = recipe_match.filter(CocktailRecipe.lab_id == target_lab_id)
        ...
        if recipe_match:
            ...
            components = db.query(CocktailRecipeComponent).filter(
                CocktailRecipeComponent.recipe_id == recipe_match.id
            ).order_by(CocktailRecipeComponent.ordinal).all()
            ab_ids = {c.antibody_id for c in components if c.antibody_id}
            abs_map = {a.id: a for a in db.query(Antibody).filter(Antibody.id.in_(ab_ids)).all()} if ab_ids else {}

            recipe_out = CocktailRecipeOut.model_validate(recipe_match, from_attributes=True)
            for comp_out in recipe_out.components:
                if comp_out.antibody_id:
                    ab = abs_map.get(comp_out.antibody_id)
                    if ab:
